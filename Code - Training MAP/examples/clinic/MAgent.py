...
# import os
# 
# os.environ["METAGPT_CONFIG_DIR"] = "/root/capsule/code/examples/clinic/.metagpt"
import subprocess
import sys

def install_metagpt():
    try:
        print("Upgrading metagpt...")
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "metagpt"])
        print("Upgrade successful!")
    except subprocess.CalledProcessError as e:
        print(f"Error upgrading metagpt: {e}")
        raise
install_metagpt()
...
config_content = """llm:
  ...
  base_url: "http://175.45.13.117:53999/v1"
  api_key: '32de14a039364bafb28799119ab5781c'
  model: "internlm2"
"""


config_path = "/root/.metagpt/config2.yaml"


config_dir = os.path.dirname(config_path)
if not os.path.exists(config_dir):
    os.makedirs(config_dir, exist_ok=True)
    print(f"Dir path： {config_dir} ")
...
if os.path.exists('doc_actions'):
    print(f"doc_actions contents: {os.listdir('doc_actions')}")
else:
    print("doc_actions directory not found!")
...
model_id = "LLaMA3_8B_Instruct_10_08"
...
def end_simulation(department, diagnostic, runtime="1 minutes and 7 seconds"):

    end_time = datetime.now()  

    print("\n" + "=" * 50)
    print("🎉 Surgery Simulation Complete!")
    print(f"🕒 Completed at: {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print("\nSummary of the operation:")
    print(f"- Assigned Department: {department}")
    print(f"- Final Diagnostic Report: {diagnostic}")
...
*                                             *
*            🏥 Operation Complete!           *
*   The patient is now stable. Thank you!     *
*                                             *
***********************************************
""")
    print("👉 Next Steps:")
    print("- Transfer the patient to the recovery unit.")
    print("- Monitor vital signs for the next 24 hours.")
    print("- Schedule a follow-up consultation.")
    print("=" * 50)
...
async def clinic_system(record:str, investment:int=3.0, n_round:int=2):
    ...
    for item in data:
        record = item["record"]  
        print("=" * 50)
        print("🚀 Step 1: Preparing assignment department")
        print("=" * 50)
        # idea = idea + rag_answer
        await Assignment_department(record, investment, 3)
        department_list = department()
        print("-" * 50)
        print(f"🏥 Assigned Departments: {department_list}")
        print("-" * 50)
        # print(department_list02)
        # department_list = ["Medical Intensive Care Unit (MICU)"]
        for item in department_list:
            print("=" * 50)
            print(f"🔍 Processing Department: {item}")
            print("=" * 50)
            if item == "Medical Intensive Care Unit" or "Medical Intensive Care Unit (MICU)":
                ...
                print("⚠️No department has been selected yet!")

        print("=" * 50)
        print("📂 Step 2: Loading dynamic memory for final summary")
        print("=" * 50)
        file_path = dynamic_memory_path
        ...
        if summary and len(summary) > 0:
            summary = summary[-1]["role"] +"'s final diagnostic:" + summary[-1]["content"]
            print(f"✅ Final Diagnostic Summary: {summary}")
        else:
            summary = "No diagnostic summary available"
            print("⚠️ No diagnostic summary available!")
            print("-" * 50)
        await FinalDecison(str(summary), investment, 1)
        print("=" * 50)
        print("🧹 Step 3: Clearing dynamic memory")
        print("=" * 50)
        list = []

        with open(file_path, "w") as json_file:
            json.dump(list, json_file)
        print("✅ Dynamic memory cleared!")
        print("=" * 50)
        print("\n\n")
        end_simulation(
            department=f"{department_list}",
