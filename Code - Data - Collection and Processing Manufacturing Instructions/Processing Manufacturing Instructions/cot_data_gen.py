...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--datapath", type=str, default="./metadata.jsonl", help="The path at which the document is located")
    parser.add_argument("--output", type=str, default="./Output", help="The path at which to save the dataset")
    ...
    parser.add_argument("--chunk_size", type=int, default=512, help="The size of each chunk in number of tokens for each case")
    parser.add_argument("--doctype", type=str, default="json", help="The type of the document, must be one of the accepted doctypes", choices=["pdf", "txt", "json", "api"])
    parser.add_argument("--openai_key", type=str, default="your_api_token", help="Your OpenAI key used to make queries")
...
#         text_splitter = SemanticChunker(AzureOpenAIEmbeddings(openai_api_key='26c986a1f28b4d82b6b53bae3cfad48f',azure_endpoint = "https://new4v-cair.openai.azure.com/",azure_deployment="ada"
# ), number_of_chunks=num_chunks)
        text_splitter = RecursiveCharacterTextSplitter(chunk_size = 512,chunk_overlap  = 52,length_function = len,)
...
def generate_instructions_gen(chunk: Any, x: int = 5) -> list[str]:
    ...
    response = client.chat.completions.create(
        model="your_api_model",
        messages=[
            {"role": "system", "content": "You are a synthetic question-answer pair generator. Given a chunk of context about some topic(s), generate %s example questions a user could ask and would be answered using information from the chunk in English. For example, an example question could be 'What are the treatments for transnasal pituitary tumors?'" % (x)},
            {"role": "system", "content": "The questions should be able to be answered in a few words or less in English. Include only the questions in your response."},
...
def encode_question_gen(question: str, chunk: Any) -> list[str]:
    ...
    prompt = """
        ...
        - The content must not contain content about the hate, sexual, violence, and self-harm categories.  
    ...
    prompts.append({"role": "system", "content": "You are a helpful clinical expert who can provide an answer given a question and relevant medical context."})
    prompts.append({"role": "user", "content": prompt})
    return prompts
...
def get_file_names_without_extension(directory, extension):
    ...
    for file in os.listdir(directory):
        if file.endswith(extension):
            file_name = file.rstrip(extension)
...
if __name__ == "__main__":
    ...
    # client = OpenAI(
    # azure_endpoint = "https://api.chatanywhere.tech/v1", 
    # api_key='',  
    ...
    with open('./metadata.jsonl', 'r') as f:
        for line in f:
            ...
            print(f"file {line} begin")

            json_file_path = './' + str(num) + '.json'
            jsonl_file_path = './' + str(num) + '.jsonl'

            CHUNK_SIZE = 3500
            NUM_DISTRACT_DOCS = 5
            chunks = get_chunks(file_path, "txt", CHUNK_SIZE, OPENAPI_API_KEY)
            ds = None

            for i,chunk in enumerate(chunks):
                print(f"Processing No.{i} chunk")
                add_chunk_to_dataset(chunks, chunk, "txt", args.questions, NUM_DISTRACT_DOCS)

                ds.save_to_disk(args.output)
            
                # Save as .jsonl format
                # dataset_dict = ds.to_json(args.output + ".jsonl")
